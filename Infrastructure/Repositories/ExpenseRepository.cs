...
public class ExpenseRepository : IExpenseRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        return _context.Expenses
            .IncludeEverything()
            .Where(delegate(Expense e) { return e.ExpenseParticipants.Any(e => e.CustomerId == customerId); })
            .Skip(skipCount)
            .Take(takeCount)
            .ToList();
    ...
    public async Task<IReadOnlyCollection<Expense>> GetAllExpensesAsync(CancellationToken cancellationToken = default)
    {
        return await _context.Expenses.IncludeEverything().ToListAsync(cancellationToken);
    }
}